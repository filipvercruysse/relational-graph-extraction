...
def _draw_one_graph(G: nx.DiGraph, ax, title: str, pos=None):
    ...
    if edge_labels:
        nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels,
                                     ax=ax, font_size=9, font_color="#34495e")
...
def draw_comparison(graphs: list[tuple[nx.DiGraph, str, Score | None, Score | None]],
    ...
    """Draw N graphs in a vertical stack with score annotations beneath each."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    n = len(graphs)
    fig, axes = plt.subplots(n, 1, figsize=(20, 16 * n))
    fig.patch.set_facecolor("white")
    if n == 1:
        axes = [axes]

    for ax, (G, title, ent_s, rel_s) in zip(axes, graphs):
        _draw_one_graph(G, ax, title)
        subtitle_parts = []
        if ent_s:
